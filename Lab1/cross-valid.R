...
# --- The function for cross validation
# --- with fixed tau; cross validate over number of iterations
# --- the first covariate is the anchor
#     Written by Shuangge Ma (joint with Jian Huang), Dec. 2005
###################################################################

ind<-read.table("indicator.txt")$V1;
z.dat<-read.table("covariate.txt");

sigma.n<-0.50; # --- sigma.n selected using another program
anchor<-1;
V<- 5; step<- 1000; increase<-0.01; tau<-1.0;
...
write.table(cbind(seq(from=1, to=step, by=1), criteria), 
      file="cv-score.txt", append=F, quote=F, row.name=F, col.name=F);

ss<-seq(from=1, to=step, by=1);
step.est<-ss[criteria==max(criteria)];

write.table(step.est, file="cross-step.txt", append=T, quote=F, 
