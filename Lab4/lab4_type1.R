slr.experiment<-function(n.trials=100,n.data.points=10,slope=0,alpha=.05,equal.variance=TRUE){
  # This function simulates datasets for a regression model where the true
  # slope is zero.  Each simulated data set is used to test the
  # null hypothesis H0:beta1=0 against the two-sided alternative.
  # The significance level of the test is determined by the user.
  # If the option 'equal.variance' is set to TRUE, then all of 
  # the simulated data have the same variance.  If 'equal.variance' is set to
  # FALSE, then the standard deviation of the error increases as the value of 
  ...
  # equal.variance: A logical variable that is either 'TRUE' or 'FALSE'
  ...
  for(i.trial in 1:n.trials){    # start a loop
    ...
    if(equal.variance==TRUE) errors <- rnorm(n.data.points,sd=1)
    if(equal.variance==FALSE) errors <- rnorm(n.data.points,sd=x.values^2)
    # above two commands generate the errors based on whether 'equal variance' 
  ...
  if (equal.variance==TRUE) print("The equal variance assumption was satisfied.")
  else print("The equal variance assumption was violated.") 
